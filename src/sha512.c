...
static inline uint8_t *padd1024		(uint8_t *M, uint64_t *const sz);
#else
__attribute__((always_inline, nonnull, warn_unused_result, optimize("Ofast")))
static inline uint8_t *padd1024		(uint8_t *M, uint32_t *const sz);
#endif

#ifdef ARCHITECTURE64
__attribute__((always_inline, nonnull, warn_unused_result, optimize("Ofast")))
static inline uint8_t **prsng1024	(uint8_t * const M, const uint64_t sz, uint64_t *const N);
#else
__attribute__((always_inline, nonnull, warn_unused_result, optimize("Ofast")))
static inline uint8_t **prsng1024	(uint8_t * const M, const uint32_t sz, uint32_t *const N);
#endif

__attribute__((always_inline, nonnull, pure, hot, optimize("Ofast")))
static inline void schedule		(uint64_t *const, const uint8_t *const);
...
#else
__attribute__((always_inline, nonnull, warn_unused_result, optimize("Ofast")))
static inline uint8_t *padd1024 (uint8_t *M, uint32_t *const sz) /*fix this part, if sz is too big, the (*sz)*8 part causes buffer overflow*/
#endif
{
	uint8_t r, g;
...
__attribute__((always_inline, nonnull, pure, hot, optimize("Ofast")))
static inline void schedule (uint64_t *const W, const uint8_t *const M)
{
	uint8_t i;
